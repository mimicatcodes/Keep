...
class SettingsViewController: UIViewController, MFMailComposeViewControllerDelegate, UIGestureRecognizerDelegate {
    ...
    func alertForNoMail(){
        let alertController = UIAlertController(title: Email.noMail,  message: Email.unableToEmailMessage, preferredStyle: .alert)
        
        let okAction = UIAlertAction(title: Email.ok, style: .cancel, handler: { action in
